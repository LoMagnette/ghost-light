...
| Art | 🔄 | Robots are modelled in code; no animation yet |
| Audio | ⬜ | Nothing yet |
...
| **Robot animation**: idle, walk, run, one ability each | — | Robots are static primitive groups | Move the parts of each group; no rigging needed | Agent |
...
| Check the frame rate on real hardware (`F1`) | Human | — | The sandbox renders in software, so no number measured here counts |
...
| Art pass: whatever is left after animation | Human · agent | Sun 27 Sep | |
...
gantt
    ...
    section Polish
    Robot animation                   :a1, 2026-09-26, 2d
